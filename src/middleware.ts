...
//     if (sessionExpired) {
//         console.log(`Session expired for user: ${session.userId} at ${new Date().toISOString()}`);
...
export default async function middleware(req: NextRequest) {
    ...
    if (!cookie) {
        ...
        return NextResponse.next(); // Allow public route access
    }
    ...
    if (sessionExpired) {
        ...
        (await cookies()).delete("session"); // Clear expired session
